...
if __name__ == '__main__':
    ...
    parser.add_argument("--fairness_strategy", type=str,
                        choices=['FairCo', 'MCFair',"Randomk","FairK",\
                            "ExploreK","Topk","FARA","FARA-Horiz.","ILP","LP","MMF","PLFair","TaxRankIncome","didrf"],
                        # default="MCFair",  
                        default="didrf", 
                        help="fairness_strategy")
    ...
    parser.add_argument("--income_simulation", type=str,
                        choices=["realistic", "constant", "legacy", "criteo_cva", "yoochoose_rpv"],
                        default="realistic",
                        help="unit-income simulator used by f_d")
    ...
    parser.add_argument("--n_iteration", type=int,
                    default=10**4,
                    help="number of ranking iterations to simulate")
    parser.add_argument("--n_futureSession", type=int,
                    default=100,
                    help="number of future sessions used by future-aware baselines")
    ...
    parser.add_argument("--LogTimeEachStep",  type=str2bool, nargs='?',
                ...
                help="log timing for each evaluation step.")
    args = parser.parse_args()
    sim.set_income_simulation(args.income_simulation)
    income_source = sim.income_simulation_source(args.income_simulation)
    print("income_simulation_source", income_source, flush=True)
    ...
    OutputDict["income_simulation_source"].append(income_source)
    ...
    for iteration in iterationsGenerator:
        ...
        if iteration in evalIterations:
            print("current iteration"+str(iteration), flush=True)
        ...
        dataSplit.updateStatistics(
            qid,
            clicks,
            ranking,
            positionBias,
            sim.f_d(iteration, item_ids=ranking, query_id=qid),
        )
    ...
    logPath=os.path.join(args.log_dir, "result.jjson")
    print('Writing results to %s' % logPath)
