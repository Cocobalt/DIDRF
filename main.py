...
if __name__ == '__main__':
    ...
    parser.add_argument("--fairness_strategy", type=str,
                        choices=['FairCo', 'MCFair',"Randomk","FairK",\
                            "ExploreK","Topk","FARA","FARA-Horiz.","ILP","LP","MMF","PLFair","didrf"],
                        default="didrf",   
    ...
    for iteration in iterationsGenerator:
        ...
        if args.fairness_strategy == "didrf":
            current_disparity = rnk.disparity(
                dataSplit.query_values_from_vector(qid, dataSplit.exposure),
                dataSplit.query_values_from_vector(qid, dataSplit.getEstimatedAverageRelevance()),
            )
        ...
        dataSplit.updateStatistics(qid,clicks,ranking,positionBias,sim.f_d(iteration))
