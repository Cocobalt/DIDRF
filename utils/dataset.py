...
ONLINE_RELEVANCE_PRIOR = 0.5
ONLINE_RELEVANCE_PRIOR_STRENGTH = 1.0
...
class DataFoldSplit(object):
  def __init__(self, datafold, name, doclist_ranges, feature_matrix, label_vector,queryLeastLength=0,rankListLength=5, relvance_strategy=None):
    ...
    self.observedExposure=np.zeros_like(label_vector).astype(np.float64)
    self.query_freq=np.zeros(self.num_queries_orig())
    self.cacheLists=defaultdict(list)
    self.docFreq=np.zeros_like(label_vector)
    self.weightedClicksAver=np.full(label_vector.shape, ONLINE_RELEVANCE_PRIOR, dtype=np.float64)
  ...
  def updateStatistics(self,qid,clicks,ranking,positionBias,f_d):
    ...
    observedExposure=self.query_values_from_vector(qid,self.observedExposure)
    ...
    np.add.at(exposure,ranking,positionBias*f_d)
    np.add.at(observedExposure,ranking,positionBias)
    np.add.at(ClickSum,ranking,clicks)
    # np.add.at(self.weightClicksSum,ranking,clicks/positionBias)
    if observedExposure.sum() > 1e-12:
        query_prior = ClickSum.sum() / observedExposure.sum()
        query_prior = np.clip(query_prior, 0.0, 1.0)
    else:
        query_prior = ONLINE_RELEVANCE_PRIOR
    weightedClicksAver[:]=np.clip(
        (ClickSum + ONLINE_RELEVANCE_PRIOR_STRENGTH * query_prior)
        / np.clip(observedExposure + ONLINE_RELEVANCE_PRIOR_STRENGTH, 1e-5, np.inf),
        0.0,
        1.0,
    )
...
class DataFold(object):
  ...
  def _read_file(self, path, feat_map, purge):
    ...
    for line in progressbar(content):
      line_body = line.split('#', 1)[0] if '#' in line else line
      info = line_body.split()
      if not info:
        continue
      qid = info[1].split(':')[1]
      label = int(info[0])
      ...
      for pair in feat_pairs:
        if ':' not in pair:
          continue
        feat_id, feature = pair.split(':', 1)
        if not feat_id or not feature:
          continue
