...
def income_fairness_gradient(income, relevance):
    """
    Gradient of negative pairwise income disparity with respect to cumulative
    income. Positive values mean the item is under-served relative to relevance.
    """
    relevance = np.clip(relevance.astype(np.float64), 0.0, np.inf)
    income = np.clip(income.astype(np.float64), 0.0, np.inf)
    doc_num = relevance.shape[0]
    if doc_num <= 1:
      return np.zeros_like(relevance, dtype=np.float64)
    A = np.dot(income, relevance)
    B = np.dot(relevance, relevance)
    kappa = 4.0 / (doc_num * (doc_num - 1))
    return kappa * (relevance * A - income * B)


def item_unit_income(time, num_item, query_id=0):
    unit_income = sim.f_d(
      time,
      item_ids=np.arange(num_item),
      query_id=query_id,
    )
    if np.ndim(unit_income) == 0:
      return np.full(num_item, float(unit_income), dtype=np.float64)
    unit_income = np.asarray(unit_income, dtype=np.float64)
    if unit_income.shape[0] != num_item:
      unit_income = np.resize(unit_income, num_item)
    return np.clip(unit_income, 0.0, np.inf)


def income_mcfair_score(qRel, income, time, fairness_tradeoff_param,
  exploration_strategy, exploration_tradeoff_param, query_id=0):
    """
    First-order income-fairness baseline.

    This is the income-fair counterpart of MCFair: it uses the same pairwise
    income disparity objective as DIDRF, but keeps only the first-order marginal
    term mu_d(t) * grad_d and ignores cross-item second-order interactions.
    """
    num_item = qRel.shape[0]
    rel = np.clip(qRel.astype(np.float64), 0.0, np.inf)
    income = np.clip(income.astype(np.float64), 0.0, np.inf)
    gamma = max(float(fairness_tradeoff_param), 0.0)
    eta = max(float(exploration_tradeoff_param), 0.0)

    mu = item_unit_income(time, num_item, query_id=query_id)
    fair_gain = mu * income_fairness_gradient(income, rel)
    fair_scale = np.max(np.abs(fair_gain)) if fair_gain.size else 0.0
    if fair_scale > 1e-12:
      fair_gain = fair_gain / fair_scale
    else:
      fair_gain = np.zeros_like(fair_gain)

    uncertainty = uncertaintyDoc(income, exploration_strategy)
    return rel + gamma * fair_gain + eta * uncertainty


def taxrank_income_ranking(qRel, income, time, fairness_tradeoff_param,
  exploration_strategy, exploration_tradeoff_param, positionBias,
  rankListLength, query_id=0):
    """
    Income-fair adaptation of Tax-rank.

    Tax-rank treats fair re-ranking as a taxation-style welfare adjustment. In
    our income-fair setting, cumulative income is the taxed resource and
    relevance is the entitlement weight. We use the proportional-fair
    log-welfare form:

      W(I; r) = sum_d r_d log(I_d + eps).

    The ranking is built greedily by the rank-wise marginal welfare gain induced
    by the income increment mu_d(t) * p_j. This keeps the baseline O(km) and
    makes it directly compatible with the existing income simulator.
    """
    num_item = qRel.shape[0]
    if rankListLength <= 0:
      return np.array([], dtype=int)
    rel = np.clip(qRel.astype(np.float64), 0.0, np.inf)
    current_income = np.clip(income.astype(np.float64), 0.0, np.inf).copy()
    gamma = max(float(fairness_tradeoff_param), 0.0)
    eta = max(float(exploration_tradeoff_param), 0.0)
    mu = item_unit_income(time, num_item, query_id=query_id)
    selected = np.zeros(num_item, dtype=np.bool_)
    ranking = []
    eps = 1e-6

    for rank_id in range(min(rankListLength, num_item)):
      position_gain = positionBias[rank_id]
      delta_income = mu * position_gain
      welfare_gain = rel * (
        np.log(current_income + delta_income + eps)
        - np.log(current_income + eps)
      )
      valid_gain = welfare_gain[~selected]
      fair_scale = np.max(np.abs(valid_gain)) if valid_gain.size else 0.0
      if fair_scale > 1e-12:
        welfare_gain = welfare_gain / fair_scale
      else:
        welfare_gain = np.zeros_like(welfare_gain)

      uncertainty = uncertaintyDoc(current_income, exploration_strategy)
      score = position_gain * rel + gamma * welfare_gain + eta * uncertainty
      score = np.where(selected, -np.inf, score)
      selected_id = int(np.argmax(score))
      if not np.isfinite(score[selected_id]):
        break
      ranking.append(selected_id)
      selected[selected_id] = True
      current_income[selected_id] += delta_income[selected_id]
...
#     print(relevance_esti.dtype,obs.dtype,FutureFairExpo.dtype,positionBias.dtype)
    ...
    if exploration_tradeoff_param<=0:
      x = solve_qp(A, B, G=G,h=h, A=Constant_A, b=FutureFairExpo,lb=lb,ub=ub,solver="quadprog")
    else:
...
#     # print(QuotaEachItem,q_rel/(q_rel.sum()+1e-5)*FutureFairExpo)
#     return QuotaEachItem.astype(np.float)
def getQuotaEachItemNDCG(time,obs,q_rel,positionBias,n_futureSession,fairness_tradeoff_param,exploration_tradeoff_param=0): 
    """
    This funciton calcultes the Quota each item should get to keep fairness.
    """
    ...
    #     FutureFairExpo += positionBias.sum() * sim.f_d(i + time)
...
def getVerticalRankingPartial(q_rel,rankListLength,n_futureSession,ExpoBackwardCum,QuotaEachItem,positionBias):
    ...
    return rankLists
def getVerticalRanking(time, q_rel,rankListLength,n_futureSession,QuotaEachItem,positionBias):
    """
    This funciton outputs ranklists by an vertical way.
    """
    Expo=np.zeros_like(q_rel)
    ...
    for j in range(rankListLength):
        for i in range(n_futureSession):
            ...
            QuotaEachItem=np.clip(QuotaEachItem,0,np.inf)
            ranking.append(item_ij)
            Expo[item_ij]+=positionBias[j]*sim.f_d(i+time)
    return rankLists
...
def getFutureRankingQuota(obs,q_rel,positionBias,n_futureSession,rankListLength,fairness_tradeoff_param):
    ...
    return rankLists
def getFutureRankingNDCG(time, obs,q_rel,positionBias,n_futureSession,rankListLength,fairness_tradeoff_param,exploration_tradeoff_param):
    """
    This funciton outputs future rankists vertically by conidering fair exposure gurantee.
    """    
    QuotaEachItem=getQuotaEachItemNDCG(time,obs,q_rel,positionBias,n_futureSession,fairness_tradeoff_param,exploration_tradeoff_param)
    # ExpoBackwardCum=getExpoBackwardCum(n_futureSession,rankListLength,positionBias)
    rankLists=getVerticalRanking(time,q_rel,rankListLength,n_futureSession,QuotaEachItem,positionBias)
    random.shuffle(rankLists)
    return rankLists
def getFutureRankingNDCGHorizontal(time,obs,q_rel,positionBias,n_futureSession,rankListLength,fairness_tradeoff_param,exploration_tradeoff_param):
    """
    ...
    random.shuffle(rankLists)
    return rankLists
def get_rankingFromDatasplit(qid,dataSplit,f_d,**kwargs):
    """
    ...
    kwargs["qid"]=qid
    fairness_strategy = kwargs.get("fairness_strategy")
    needs_rank_freq = fairness_strategy == "MMF" or (
      fairness_strategy == "didrf"
      and (
        kwargs.get("relvance_strategy") == "EstimatedAverage"
        or float(kwargs.get("exploration_tradeoff_param", 0.0)) > 0.0
      )
    )
    if needs_rank_freq:
      kwargs["ItemFreqEachRank"]=dataSplit.query_values_from_vector(qid,dataSplit.ItemFreqEachRank)
    ranking=get_ranking(qRel,qExpVector,f_d,cacheLists=cacheLists,dataSplit=dataSplit,\
...
def _center_normalize_available_gain(gain, selected, available_count=None,
  abs_buffer=None, centered_buffer=None):
    if available_count is None:
      available_count = int(np.sum(~selected))
    if available_count <= 0:
      return np.zeros_like(gain)

    gain_mean = np.sum(gain, where=~selected, initial=0.0) / float(available_count)
    if centered_buffer is None:
      centered = gain - gain_mean
    else:
      np.subtract(gain, gain_mean, out=centered_buffer)
      centered = centered_buffer

    if abs_buffer is None:
      valid_centered = centered[~selected]
      gain_scale = np.max(np.abs(valid_centered)) if valid_centered.size else 0.0
    else:
      np.abs(centered, out=abs_buffer)
      gain_scale = np.max(abs_buffer, where=~selected, initial=0.0)

    if gain_scale > 1e-12:
      return 0.5 * centered / gain_scale
    return np.zeros_like(gain)


def _max_normalize_available_gain(gain, selected, abs_buffer=None):
    if abs_buffer is None:
      valid_gain = gain[~selected]
      gain_scale = np.max(np.abs(valid_gain)) if valid_gain.size else 0.0
    else:
      np.abs(gain, out=abs_buffer)
      gain_scale = np.max(abs_buffer, where=~selected, initial=0.0)
    if gain_scale > 1e-12:
      return 0.5 * gain / gain_scale
    return np.zeros_like(gain)


def _winsorize_positive_signal(values, upper_quantile=0.95):
    values = np.clip(np.asarray(values, dtype=np.float64), 0.0, np.inf)
    positive = values[values > 1e-12]
    if positive.size <= 1:
      return values.copy()
    cap = float(np.quantile(positive, upper_quantile))
    if cap <= 1e-12:
      return values.copy()
    return np.minimum(values, cap)


def _sensitivity_controlled_income(values, mode=None):
    values = np.clip(np.asarray(values, dtype=np.float64), 0.0, np.inf)
    if mode is None:
      return values
    positive = values[values > 1e-12]
    if positive.size == 0:
      return values.copy()
    reference = float(np.mean(positive))
    if reference <= 1e-12:
      return values.copy()
    if mode == "sqrt":
      return np.sqrt(values * reference)
    if mode == "log":
      return reference * np.log1p(values / reference) / np.log(2.0)
    raise ValueError("Unknown income sensitivity transform: %s" % mode)


def _ndcg_idcg(rel, positionBias, rankListLength):
    cutoff = min(rankListLength, rel.shape[0], positionBias.shape[0])
    if cutoff <= 0:
      return 0.0
    if cutoff < rel.shape[0]:
      ideal_ids = np.argpartition(-rel, cutoff - 1)[:cutoff]
      ideal_values = np.sort(rel[ideal_ids])[::-1]
    else:
      ideal_values = np.sort(rel)[::-1]
    return float(np.dot(ideal_values, positionBias[:cutoff]))


def _ndcg_utility_relevance(rel, positionBias, rankListLength, utility_scale=None):
    if utility_scale is not None:
      return rel * float(utility_scale)
    idcg = _ndcg_idcg(rel, positionBias, rankListLength)
    if idcg <= 1e-12:
      return rel
    return rel / idcg
...
def _observed_exposure_from_rank_counts(item_freq_each_rank, positionBias, fallback):
    if item_freq_each_rank is None:
      return fallback
    usable_rank_len = min(item_freq_each_rank.shape[1], positionBias.shape[0])
    if usable_rank_len <= 0:
      return fallback
    return np.sum(item_freq_each_rank[:, :usable_rank_len] * positionBias[:usable_rank_len][None, :], axis=1)


def _didrf_state_pressure_ranking(rel, utility_rel, income, fair_mu, g, kappa,
  B, gamma, positionBias, rankListLength):
    """
    State-calibrated DIDRF optimizer.

    The score keeps the first-order income residual and the second-order
    prefix interaction, then scales the fairness pressure by the current
    residual income-unfairness state.
    """
    num_item = rel.shape[0]
    if rankListLength <= 0:
      return np.array([], dtype=int)
    if B <= 1e-12:
      return single_ranking(utility_rel, rankListLength=rankListLength)

    eps = 1e-12
    a = rel * fair_mu
    b = fair_mu * g
    c = fair_mu * fair_mu
    rel_curvature = np.clip(B - rel * rel, 0.0, np.inf)

    A = float(np.dot(income, rel))
    phi = 0.5 * kappa * (B * float(np.dot(income, income)) - A * A)
    phi = max(0.0, phi)

    selected = np.zeros(num_item, dtype=np.bool_)
    ranking = []
    prefix_interaction = 0.0
    fairness_accumulator = 0.0
    utility_accumulator = 0.0

    for rank_id in range(rankListLength):
      position_gain = positionBias[rank_id]
      self_curvature = (
        0.5
        * kappa
      ...
      fair_step = (
        position_gain * b
        + kappa * prefix_interaction * a * position_gain
        - self_curvature
      )
      rho = max(float(np.max(self_curvature)) if self_curvature.size else 0.0, eps)
      sqrt_phi = np.sqrt(max(phi, 0.0))
      pressure = 0.5 + sqrt_phi / (sqrt_phi + np.sqrt(rho) + eps)

      candidate_fairness = fairness_accumulator + pressure * fair_step
      candidate_utility = utility_accumulator + position_gain * utility_rel
      candidate_score = candidate_utility + gamma * candidate_fairness
      candidate_score[selected] = -np.inf

      selected_id = int(np.argmax(candidate_score))
      if not np.isfinite(candidate_score[selected_id]):
        break

      ranking.append(selected_id)
      selected[selected_id] = True
      prefix_interaction += a[selected_id] * position_gain
      fairness_accumulator = float(candidate_fairness[selected_id])
      utility_accumulator = float(candidate_utility[selected_id])
      phi = max(0.0, phi - float(fair_step[selected_id]))

    return np.array(ranking, dtype=int)

def didrf_interaction_aware_ranking(qRel, income, time, fairness_tradeoff_param,
  exploration_strategy, exploration_tradeoff_param, rankListLength, positionBias,
  item_freq_each_rank=None, query_id=0, online_relevance=False):
    """
    DIDRF ranking policy used in the submitted code.

    The state variable named exposure is treated as cumulative income here,
    because updateStatistics adds positionBias * f_d(time) to it. The policy
    uses the income-fair first-order residual, the second-order prefix
    interaction, and a state-dependent fairness pressure. The online setting
    uses the same income-fairness score with an optimistic relevance utility.
    """
    num_item = qRel.shape[0]
    rankListLength = min(rankListLength, num_item, positionBias.shape[0])
    if rankListLength <= 0:
      return np.array([], dtype=int)
    if num_item <= 1:
      return np.arange(num_item, dtype=int)[:rankListLength]

    rel = np.clip(qRel.astype(np.float64), 0.0, np.inf)
    income = np.clip(income.astype(np.float64), 0.0, np.inf)
    gamma = max(float(fairness_tradeoff_param), 0.0)

    unit_income = sim.f_d(time, item_ids=np.arange(num_item), query_id=query_id)
    if np.ndim(unit_income) == 0:
      fair_mu = np.full(num_item, float(unit_income), dtype=np.float64)
    else:
      fair_mu = np.asarray(unit_income, dtype=np.float64)
      if fair_mu.shape[0] != num_item:
        fair_mu = np.resize(fair_mu, num_item)
    fair_mu = np.clip(fair_mu, 0.0, np.inf)

    utility_rel = rel.copy()
    observed_exposure = income
    if online_relevance:
      observed_exposure = _observed_exposure_from_rank_counts(
        item_freq_each_rank, positionBias, fallback=income)
      confidence = np.sqrt(
        rel * (1.0 - rel)
        / np.clip(observed_exposure + 1.0, 1.0, np.inf)
      )
      utility_rel = np.clip(utility_rel + confidence, 0.0, np.inf)

    kappa = 4.0 / (num_item * (num_item - 1))
    A = np.dot(income, rel)
    B = np.dot(rel, rel)
    if B <= 1e-12:
      return single_ranking(utility_rel, rankListLength=rankListLength)
    g = kappa * (rel * A - income * B)

    return _didrf_state_pressure_ranking(
      rel=rel,
...
def didrf_fairness_only_ranking(qRel, income, time, rankListLength, positionBias,
  query_id=0):
    """
    Fairness-only DIDRF greedy ranking.

    This uses the same dynamic income-fairness marginal term as DIDRF, including
    the item-specific income factor f_d(t) and the second-order prefix
    interaction, but removes the relevance utility term.
    """
    num_item = qRel.shape[0]
    if positionBias is None:
      positionBias = np.ones(rankListLength, dtype=np.float64)
    rankListLength = min(rankListLength, num_item, positionBias.shape[0])
    if rankListLength <= 0:
      return np.array([], dtype=int)
    if num_item <= 1:
    ...
    if np.ndim(unit_income) == 0:
      mu = np.full(num_item, float(unit_income), dtype=np.float64)
    else:
      mu = np.asarray(unit_income, dtype=np.float64)
      if mu.shape[0] != num_item:
        mu = np.resize(mu, num_item)
    mu = np.clip(mu, 0.0, np.inf)

    kappa = 4.0 / (num_item * (num_item - 1))
    A = np.dot(income, rel)
    B = np.dot(rel, rel)
    if B <= 1e-12:
      return single_ranking(rel, rankListLength=rankListLength)

    g = kappa * (rel * A - income * B)
    a = rel * mu
    h = -kappa * (B - rel * rel) * mu * mu
    linear_fair = mu * g

    selected = np.zeros(num_item, dtype=np.bool_)
    ranking = []
    prefix_interaction = 0.0
    fair_abs = np.empty_like(rel, dtype=np.float64)

    for rank_id in range(rankListLength):
      position_gain = positionBias[rank_id]
      fair_gain = (
        position_gain * linear_fair
        + kappa * prefix_interaction * a * position_gain
        + 0.5 * h * position_gain * position_gain
      )
      fair_gain = _max_normalize_available_gain(
        fair_gain,
        selected,
        abs_buffer=fair_abs,
      )
      marginal_gain = fair_gain + 1e-12 * rel
      marginal_gain[selected] = -np.inf

      selected_id = int(np.argmax(marginal_gain))
      selected[selected_id] = True
      ranking.append(selected_id)
      prefix_interaction += a[selected_id] * position_gain
...
def get_ranking(qRel,qExpVector,time,fairness_strategy,fairness_tradeoff_param,exploration_strategy,exploration_tradeoff_param,\
  rankListLength,n_futureSession=None,positionBias=None,cacheLists=[],queryFreq=0,dataSplit=None,**kwargs):

    num_item=qRel.shape[0]
    if fairness_strategy == "MCFair":
      RankingScore = income_mcfair_score(
        qRel=qRel,
        income=qExpVector,
        time=time,
        fairness_tradeoff_param=fairness_tradeoff_param,
        exploration_strategy=exploration_strategy,
        exploration_tradeoff_param=exploration_tradeoff_param,
        query_id=kwargs.get("qid", 0),
      )
      ranking=single_ranking(RankingScore,rankListLength=rankListLength)
    elif fairness_strategy == "TaxRankIncome":
      ranking = taxrank_income_ranking(
        qRel=qRel,
        income=qExpVector,
        time=time,
        fairness_tradeoff_param=fairness_tradeoff_param,
    ...
    elif fairness_strategy in ["FairCo",'FairCo_multip.',"FairCo_maxnorm"]:
      Docunfairness=unfairnessDoc(qExpVector,qRel,fairness_strategy)
      Uncertainty=uncertaintyDoc(qExpVector,exploration_strategy)
      # RankingScore=qRel+fairness_tradeoff_param*Docunfairness*sim.f_d(time)+exploration_tradeoff_param*Uncertainty
      RankingScore=qRel+fairness_tradeoff_param*Docunfairness*sim.f_d(time)+exploration_tradeoff_param*Uncertainty
      ranking=single_ranking(RankingScore,rankListLength=rankListLength)
    elif fairness_strategy in ["onlyFairness"]:
      Docunfairness=unfairnessDoc(qExpVector,qRel,"MCFair")
      Uncertainty=uncertaintyDoc(qExpVector,exploration_strategy)
      RankingScore=Docunfairness*sim.f_d(time)+exploration_tradeoff_param*Uncertainty
      ranking=single_ranking(RankingScore,rankListLength=rankListLength)
    ...
    #   ranking=np.array(cacheLists.pop())
      ...
      return didrf_interaction_aware_ranking(
        qRel=qRel,
        income=qExpVector,
        time=time,
        fairness_tradeoff_param=fairness_tradeoff_param,
        ...
        rankListLength=rankListLength,
        positionBias=positionBias,
        item_freq_each_rank=kwargs.get("ItemFreqEachRank", None),
        query_id=kwargs.get("qid", 0),
        online_relevance=(
          dataSplit is not None
          and getattr(dataSplit, "relvance_strategy", None) == "EstimatedAverage"
        )
      )
    elif fairness_strategy in ["FARA"]:
      if len(cacheLists)<=0:
        cacheLists+=getFutureRankingNDCG(time,qExpVector,qRel,positionBias,n_futureSession,rankListLength,fairness_tradeoff_param,exploration_tradeoff_param)
      ranking=np.array(cacheLists.pop())
    elif fairness_strategy in ["FARA-Horiz."]:
      if len(cacheLists)<=0:
        cacheLists+=getFutureRankingNDCGHorizontal(time,qExpVector,qRel,positionBias,n_futureSession,rankListLength,fairness_tradeoff_param,exploration_tradeoff_param)
      ranking=np.array(cacheLists.pop())
      ...
      ranking=didrf_fairness_only_ranking(
        qRel=qRel,
        income=qExpVector,
        time=time,
        rankListLength=rankListLength,
