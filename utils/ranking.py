...
def cal_secondItem(rel, exp, **kwargs):
    rel_squared = np.square(rel)
    cons = np.sum(rel_squared)
    ans = cons - rel_squared  
    q_n_docs = rel.shape[0]
    factor = -2 / (q_n_docs * (q_n_docs - 1))  
    ans *= factor
...
def get_ranking(qRel,qExpVector,time,fairness_strategy,fairness_tradeoff_param,exploration_strategy,exploration_tradeoff_param,\
  rankListLength,n_futureSession=None,positionBias=None,cacheLists=[],queryFreq=0,dataSplit=None,**kwargs):
    ...
    elif fairness_strategy in ["didrf"]:     
      Docunfairness = unfairnessDoc(qExpVector, qRel, fairness_strategy)
      Uncertainty=uncertaintyDoc(qExpVector,exploration_strategy)

      secondItem = cal_secondItem(qRel, qExpVector)

      abs_Docunfairness = np.abs(Docunfairness)
      abs_taylor2 = np.abs(secondItem)

      max_abs_Docunfairness = np.max(abs_Docunfairness) + 1e-10
      max_abs_taylor2 = np.max(abs_taylor2) + 1e-10

      totale = disparity(qExpVector, qRel,**kwargs)
      totalError = np.zeros(len(qExpVector))

      totalError[:] = totale

      v = sim.f_d(time)
      normalized_Docunfairness = Docunfairness / max_abs_Docunfairness
      normalized_taylor2 = secondItem / max_abs_taylor2
      Docunfairness = normalized_Docunfairness * v + normalized_taylor2 * v * v
      RankingScore = qRel + fairness_tradeoff_param  *  Docunfairness + exploration_tradeoff_param* (Uncertainty + 1/np.clip(qExpVector**3,0.1,np.inf))
      ranking = single_ranking(RankingScore, rankListLength=rankListLength)
